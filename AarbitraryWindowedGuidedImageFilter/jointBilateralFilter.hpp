...
{
	if (dest.empty() || dest.type() != src_.type()) dest.create(src_.size(), src_.type());
	Mat src = src_.getMat();
	Mat guide = guide_.getMat();
	Mat dst = dest.getMat();

	if (kernelType == FILTER_CIRCLE || kernelType == FILTER_DEFAULT)
	{
		if (src.depth() == CV_8U)
		{
	...
	else if (kernelType == FILTER_RECTANGLE)
	{
		if (src.depth() == CV_8U)
		{
	...
	else if (kernelType == FILTER_SEPARABLE)
	{
	...
	else if (kernelType == FILTER_SLOWEST)
	{
		Mat mask;
...
{
	jointBilateralFilter(src, guide, dest, Size(D, D), sigma_color, sigma_space, kernelType, borderType);
}